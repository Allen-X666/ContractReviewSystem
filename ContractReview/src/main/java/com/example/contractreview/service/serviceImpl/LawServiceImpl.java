...
import com.example.contractreview.constant.LawConstant;
...
import com.example.contractreview.service.LawService;
...
public class LawServiceImpl implements LawService {
    ...
    public Result<List<String>> getAllCategories() {
        String cacheKey = LawConstant.LAW_CATEGORIES_KEY;

        try {
            String cachedData = stringRedisTemplate.opsForValue().get(cacheKey);
            if (cachedData != null) {
                log.info("从Redis获取法规分类列表");
                List<String> categories = objectMapper.readValue(cachedData, new TypeReference<List<String>>() {});
        ...
        List<String> categories = lawMapper.selectAllCategories();

        try {
            String categoriesJson = objectMapper.writeValueAsString(categories);
            stringRedisTemplate.opsForValue().set(cacheKey, categoriesJson);
            log.info("法规分类列表已缓存到Redis");
        } catch (Exception e) {
    ...
    @Override
    public Result<List<LawRegulationVO>> searchLaws(String keyword, String category, Integer page, Integer pageSize) {
        String cacheKey = LawConstant.LAW_SEARCH_KEY_PREFIX + keyword + ":" + category + ":" + page + ":" + pageSize;

        try {
            String cachedData = stringRedisTemplate.opsForValue().get(cacheKey);
            if (cachedData != null) {
                log.info("从Redis获取法规搜索结果, keyword: {}, category: {}", keyword, category);
                List<LawRegulationVO> voList = objectMapper.readValue(cachedData, new TypeReference<List<LawRegulationVO>>() {});
                return Result.success(voList);
            }
        } catch (Exception e) {
            log.warn("从Redis读取法规搜索结果失败, error: {}", e.getMessage());
        }

        PageHelper.startPage(page != null ? page : 1, pageSize != null ? pageSize : 10);
        List<LawRegulation> laws = lawMapper.searchLaws(keyword, category);

        List<LawRegulationVO> voList = laws.stream()
                .map(this::convertToVO)
                .collect(Collectors.toList());

        try {
            String lawsJson = objectMapper.writeValueAsString(voList);
            stringRedisTemplate.opsForValue().set(cacheKey, lawsJson, CACHE_EXPIRE_HOURS, TimeUnit.HOURS);
            log.info("法规搜索结果已缓存到Redis");
        } catch (Exception e) {
            log.warn("缓存法规搜索结果到Redis失败, error: {}", e.getMessage());
        }

        log.info("从数据库查询法规搜索结果, keyword: {}, category: {}", keyword, category);
        return Result.success(voList);
    }

    @Override
    public Result<LawRegulationDetailVO> getLawDetail(Long lawId) {
        String cacheKey = LawConstant.LAW_DETAIL_KEY_PREFIX + lawId;

        try {
            String cachedData = stringRedisTemplate.opsForValue().get(cacheKey);
            if (cachedData != null) {
                log.info("从Redis获取法规详情, lawId: {}", lawId);
        ...
        try {
            ...
            stringRedisTemplate.opsForValue().set(cacheKey, detailJson, CACHE_EXPIRE_HOURS, TimeUnit.HOURS);
            log.info("法规详情已缓存到Redis, lawId: {}", lawId);
        } catch (Exception e) {
